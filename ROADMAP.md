...
- [x] Provide **Select all**, **Select none** and **Invert selection** controls plus an explicit selected/total counter.
- [x] Persist partial package selections by stable rule UUID so **Update all** never silently expands a previously restricted package.
- [x] Allow an installed package selection to be edited later and apply only the changed selection.
